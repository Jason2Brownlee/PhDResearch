 ...
 * </pre>
 */
public class DCCCSA_PreCommitted extends DCCCSA
{
	...
	protected LinkedList<CellSet> internalInitialiseBeforeRun(Problem problem)
	{
		...
		// pre-committed
		cells = new LinkedList[Antigen.NUM_COMPONENTS];		
		for (int i = 0; i < cells.length; i++)
		{
			int nc = numCells/cells.length;
			cells[i] = DegenerateUtils.getRandomSubCellRepertoire(rand, nc, Antigen.BITS_PER_COMPONENT);
			DegenerateUtils.assignComponentOfInterest(i, cells[i]);
		}
	...
	{	
		return compressRepertoires();
	}

	
	protected LinkedList<SubCell> compressRepertoires()
	{
		LinkedList<SubCell> master = new LinkedList<SubCell>();
		for (int i = 0; i < cells.length; i++)
		{
			master.addAll(cells[i]);
		}
		return master;
	}
	...
	{
		// assess repertoire
		DegenerateUtils.assignComponentOfInterest(component, cells[component]);
		DegenerateUtils.assessRepertoireAgainstAntigen(antigen, cells[component]);
		// select the activated set
		LinkedList<SubCell> selectedSet = CellUtils.selectCellSet(cells[component], rand, numSelected);			
		...
		CellUtils.replaceIntoRepertoireSimilarityScore(clones, cells[component], rand);
		// get the component bmu
		return CellUtils.getRepertoireBMU(cells[component], rand);
	}
