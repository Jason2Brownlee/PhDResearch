 ...
 * </pre>
 */
public class HostsAverageCellDiversity extends GenericEpochCompletedProbe
{
	...
	{	
		if(algorithm != null)
		{
			// build a master repertoire
			Host [] repertoires = algorithm.getHosts();			
			LinkedList<DiscreteCell> cells = new  LinkedList<DiscreteCell>();
			for (int i = 0; i < repertoires.length; i++)
			{
				cells.addAll(repertoires[i].getRepertoire());
			}
			
			// assess diversity of master
	...
	{
		return "Host Average Cell Diversity (HACD)";
	}
}
