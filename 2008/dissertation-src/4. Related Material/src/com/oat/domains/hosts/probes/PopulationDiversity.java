 ...
 * </pre>
 */
public class PopulationDiversity extends GenericEpochCompletedProbe
{
	...
	{	
		if(algorithm != null)
		{
			...
			{
				LinkedList<Cell> rep1 = hosts[i].getRepertoire();
				double isum = 0;
				...
				for (int j = 0; j < hosts.length; j++)
				{
					...
					LinkedList<Cell> rep2 = hosts[j].getRepertoire();
					isum += TissueUtils.interRepertoireDiversity(rep1, rep2);
				}
				
				sum += (isum / hosts.length);
			}
			
			diversity = (sum / hosts.length);
		}
	}
	...
	{
		return "Population Diversity (PD)";
	}
}
