 ...
 * </pre>
 */
public class TissueExposurePlot extends GenericProblemPlot
	implements AlgorithmEpochCompleteListener, ClearEventListener, AlgorithmChangedListener, InfectionExposureListener
{	
	...
	public void algorithmChangedEvent(Algorithm a)
	{
		...
		{			
			...
			if(a instanceof TissueAlgorithm)
			{
				...
				repertoireExposures = new LinkedList[algorithm.getNumTissues()];
				for (int i = 0; i < repertoireExposures.length; i++)
				{
					repertoireExposures[i] = new LinkedList<Color>();
				}
				
				tempRepertoireExposures = new LinkedList[algorithm.getNumTissues()];
	...
	protected void prepareProblemColours()
	{
		if(problemColours.isEmpty())
		{
			...
			{
				problemColours.add(infections[i].getGoalCoordinateColour());
			}
	...
	public void exposure(
			int repertoireNumber, Tissue repertoire,
			int patternNumber, Infection antigen)
	{
		synchronized(this)
		{
			...
			// record the exposure
			Color pat = problemColours.get(patternNumber);
			tempRepertoireExposures[repertoireNumber].add(pat);
		}
