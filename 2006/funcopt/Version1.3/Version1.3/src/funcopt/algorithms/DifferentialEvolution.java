...
public class DifferentialEvolution extends Algorithm implements ActionListener
{
    ...
    protected boolean stopOnConvergence = false;
    ...
    {        
        ...
        while(p.isReamainingEvaluations())
        {
            ...
            if(stopOnConvergence)
            {
                ...
                for (int i = 0; canStop && i < pop.size(); i++)
                {
                    Solution si = pop.get(i);
                    
                    for (int j = i; canStop && j < pop.size(); j++)
                    {
                        Solution sj = pop.get(j);
                        if(!si.equals(sj))
                        {
                            canStop = false;
                        }
    ...
    public LinkedList<Solution> generateChildren(LinkedList<Solution> pop, Problem p)
    {
        ...
        for (int i = 0; i < NP; i++)
        {
            ...
            // add child to list
            PopulationAlgorithmUtils.fixCoordBounds(child, p);
