 ...
 */
public class RankBasedAntSystem extends Algorithm
{
    ...
    {   
        ...
        try
        {
            rankSize = Integer.parseInt(rankField.getText());
        }
        catch (Exception e)
        {
            rankSize = DEFAULT_RANK;
            rankField.setText("" + rankSize);
        }
        finally
        {
    ...
    {        
        ...
        JLabel rankLabel = new JLabel("Rank size:");
        ...
        rankField = new JTextField(Integer.toString(rankSize), 10);
    ...
    protected void updatePheromone(LinkedList<Solution> ants)
    {
        // decay
        ...
        for (int i = 0; i < rankSize; i++)     
        {
            ...
            double weight = Math.max(0, (rankSize-i));
            a.updatePheromone(weight);
        }  
        // update pheromone for best solution found thus far
        ((Ant)bestEver).updatePheromone(rankSize);
    }
    ...
    protected void initialisePheromoneMatrix(Problem p, Solution nnSolution)
    {
        ...
        for (int i = 0; i < pheromoneMatrix.length; i++)
        {
            for (int j = 0; j < pheromoneMatrix[i].length; j++)
            {
                // rank based initialisation
                //pheromoneMatrix[i][j] = ((0.5*r*(1.0-r)) / (rho*nnSolution.getScore()));
                // no idea where r comes from - so use AS approach
                pheromoneMatrix[i][j] = (totalAnts / nnSolution.getScore());
            }
        }
    }
...
//            }
        ...
        public void updatePheromone(double factor, double weight)
        {
            double d = (factor / score) * weight; 
        ...
        {
            updatePheromone(1.0, weight); // default
        }
    ...
    {
        return "Rank-Based Ant System (RBAS)";
    }
}
