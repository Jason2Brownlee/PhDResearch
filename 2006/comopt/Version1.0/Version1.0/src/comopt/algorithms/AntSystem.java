 ...
 */
public class AntSystem extends Algorithm
{
    ...
    public final static double DEFAULT_ELITISM = 1.0; 
    ...
    {   
        ...
        try
        {
            elitismFactor = Double.parseDouble(elitismField.getText());
        }
        catch (Exception e)
        {
            elitismFactor = DEFAULT_ELITISM;
            elitismField.setText("" + elitismFactor);
        }
    }
    ...
    {        
        ...
        JLabel elitismLabel = new JLabel("Elitism factor:");
        ...
        elitismField = new JTextField(Double.toString(elitismFactor), 10);
    ...
    protected void updatePheromone(LinkedList<Solution> ants)
    {
        ...
        ((Ant)bestEver).updatePheromone(elitismFactor);
    }
    ...
    protected void initialisePheromoneMatrix(Problem p, Solution nnSolution)
    {
        ...
        for (int i = 0; i < pheromoneMatrix.length; i++)
        {
            for (int j = 0; j < pheromoneMatrix[i].length; j++)
            {
                ...
                {
                    pheromoneMatrix[i][j] = ((elitismFactor+totalAnts) / (rho*nnSolution.getScore()));
                }
            }
        }
    }
